...
const SideBar = (props) => {
  ...
  const numOfAppliers = props.nAppliers;
    ...
    const acceptHandler = async (applierid) => {
        const userstate = await post('/user/acceptstate', { applierid, offerid: props.offerid, companyid: props.companyid, state: 'accept' })
        if (userstate.ok) {
    ...
    const regectHandler = async (applierid) => {
        const userstate = await post('/user/acceptstate', { applierid, offerid: props.offerid, companyid: props.companyid, state: 'regect'})
        if (userstate.ok) {
          ...
          {appliers
            ? appliers.map((applier) => (
                <div key={applier._id} className={classes.applierCard}>
                    ...
                    {props.offerdetails.acceptedAppliers.some((ele) => ele._id === applier._id) ? <p>accepted</p>
                        : props.offerdetails.regectedAppliers.some((ele) => ele._id === applier._id) ? <p>regected</p> : 
                            ( <><TiUserAddOutline className={classes.add} onClick={()=>acceptHandler(applier._id)} />
                    <TiUserDeleteOutline className={classes.remove} onClick={()=>regectHandler(applier._id)} /> </>)}
                </div>
              ))
            : ""}
          {numOfAppliers == 0 ? (
